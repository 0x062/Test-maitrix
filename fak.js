...
class WalletBot {
  ...
  async swapToken(n) {
    try {
      ...
      const tokenAddr = globalConfig.tokens[n];
      const router = globalConfig.routers[n];
      const methodId = globalConfig.methodIds[`${n}Swap`];
  ...
  async stakeToken(n, overrideAddr) {
    try {
      ...
      const stakeCt = globalConfig.stakeContracts[n];
      const { balance, formatted, symbol } = await this.getTokenBalance(tokenAddr);
      if (balance.isZero()) {
  ...
  async claimFaucets() {
    ...
    for (const [key, url] of Object.entries(endpoints)) {
      try {
        ...
        console.log(`💧 ${key} faucet status: ${res.status}`);
      } catch (e) {
        console.log(`❌ faucet ${key} err:`, e.message);
      }
      await this.delay(globalConfig.delayMs);
    }
...
(async () => {
  ...
  console.log(`✨ Loaded ${keys.length} keys, ${proxies.length} proxies`);
  if (keys.length === 0) {
