...
class WalletBot{
  ...
  async stakeToken(n, c) {
    const tokenAddr = c || this.cfg.tokens[n];
    const stakeCt = this.cfg.stakeContracts[n];
    ...
    const approveTx = await contract.approve(stakeCt, balance, {
    ...
    const data = this.cfg.methodIds.stake +
      ethers.utils.defaultAbiCoder.encode(['uint256'], [balance]).slice(2);
    const stakeTx = await this.wallet.sendTransaction({ to: stakeCt, data, gasLimit: this.cfg.gasLimit, gasPrice: this.cfg.gasPrice });
