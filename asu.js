...
const erc20Abi = ['function balanceOf(address owner) view returns (uint256)', 'function decimals() view returns (uint8)', 'function approve(address spender, uint256 amount) returns (bool)', 'function symbol() view returns (string)'];
...
class WalletBot {
  ...
  async swapToken(name) {
    try {
      ...
      await new ethers.Contract(addr, erc20Abi, this.wallet).approve(router, balance, { gasLimit: this.config.gasLimit, gasPrice: this.config.gasPrice }).then(tx => tx.wait());
      await this.delay(this.config.delayMs);
      const data = id + ethers.utils.defaultAbiCoder.encode(['uint256'], [balance]).slice(2);
      await this.wallet.sendTransaction({ to: router, data, gasLimit: this.config.gasLimit, gasPrice: this.config.gasPrice }).then(tx => tx.wait());
      console.log(`✅ Swapped ${formatted} ${symbol}`);
    } catch (e) { console.error(`❌ swap ${name} failed:`, e.message); }
  }
  async stakeToken(name, custom) {
    try {
      const addr = custom || this.config.tokens[name];
      const { balance, formatted, symbol } = await this.getTokenBalance(addr);
      if (balance.isZero()) return;
      await new ethers.Contract(addr, erc20Abi, this.wallet).approve(this.config.stakeContracts[name], balance, { gasLimit: this.config.gasLimit, gasPrice: this.config.gasPrice }).then(tx => tx.wait());
      await this.delay(this.config.delayMs);
      const data = this.config.methodIds.stake + ethers.utils.defaultAbiCoder.encode(['uint256'], [balance]).slice(2);
      const tx = await this.wallet.sendTransaction({ to: this.config.stakeContracts[name], data, gasLimit: this.config.gasLimit, gasPrice: this.config.gasPrice });
      await tx.wait();
      console.log(`🎉 Staked ${formatted} ${symbol}`);
      await sendReport(`🚀 *Staked!* ${symbol} ${formatted} (${tx.hash})`);
    } catch (e) { console.error(`❌ stake ${name} failed:`, e.message); }
  }
  async checkStatus() {
  ...
  async claimFaucets() {
    const e = { ath: 'https://app.x-network.io/maitrix-faucet/faucet', usde: 'https://app.x-network.io/maitrix-usde/faucet', lvlusd: 'https://app.x-network.io/maitrix-lvl/faucet', virtual: 'https://app.x-network.io/maitrix-virtual/faucet', vana: 'https://app.x-network.io/maitrix-vana/faucet' };
    for (let [k,u] of Object.entries(e)) {
      try { await axios.post(u, { address: this.address }); console.log(`✔️ Faucet ${k}`); } catch (e) { console.error(`❌ Faucet ${k} failed`); }
      await this.delay(this.config.delayMs);
    }
  }
  async runBot() {
...
(async()=>{
  ...
  for (let k of keys) { await new WalletBot(k, globalConfig).runBot(); await new WalletBot(k, globalConfig).delay(globalConfig.delayMs); }
  console.log('🎉 All done');
  setInterval(async()=>{ for (let k of getPrivateKeys()) await new WalletBot(k, globalConfig).runBot(); }, 24*60*60*1000);
})();
